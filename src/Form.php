...
class Form
{
	...
	public function __construct($cfg = array(), EventManager $eventManager = null)
	{
		...
		*/
		...
		/*
		if (empty($this->cfg['field']['idPrefix'])) {
			$this->cfg['field']['idPrefix'] = $this->cfg['name'];
		}
		*/
	...
	/*
	public function setCompleted($bool = true)
	{
		$this->status['completed'] = $bool;
	}
	*/
	...
	/*
	public function setSubmitted($bool = true)
	{
		$this->status['submitted'] = $bool;
	}
	*/
	...
	private function redirectPost()
	{
		...
		/*
		$haveWampDebug = false;
		foreach ($this->debug->eventManager->getSubscribers('debug.log') as $subscriber) {
			if (\is_array($subscriber) && array(\get_class($subscriber[0]), $subscriber[1]) == array('bdk\\Debug\\OutputW\\amp', 'onLog')) {
				$haveWampDebug = true;
				break;
			}
		}
		if ($this->debug->getCfg('output') && !$haveWampDebug) {
			$this->debug->groupUncollapse();
		...
		$prevHandler = \set_exception_handler(function (\Exception $e) {
			$this->debug->error('exit exception');
		});
		if ($prevHandler) {
	...
	private function storeValues()
	{
		...
		/*
		if ($this->persist->get('currentPage.values')) {
			$this->debug->warn('already have values');
			$this->debug->groupEnd();
			return;
		}
		*/
	...
	/**
	 * Updates status['invalidFields']
	 *
	 * @return void
	 */
	/*
	private function updateInvalid()
	{
		$this->status['invalidFields'] = array();
		foreach ($this->currentFields as $field) {
			if (!$field->isValid) {
				$this->status['invalidFields'][] = $field->attribs['name'];
			}
		}
	}
	*/
	...
	/**
	 * Verify that passed persist key matches session key
	 * updates status[keyVerified]
	 *
	 * @return boolean
	 */
	/*
	...
	{
		$key = !empty($_REQUEST['_key_'])
			? $_REQUEST['_key_']
			: null;
		if (!$this->cfg['verifyKey']) {
			$this->status['keyVerified'] = true;
		} elseif ($key) {
			$this->status['keyVerified'] = $this->persist->verifyKey($key);
		} else {
			$this->status['keyVerified'] = false;
		}
		return $this->status['keyVerified'];
	}
	*/
}
