...
class FieldBase
{
	...
	/**
	 * [getDefinitionProps description]
	 *
	 * @param string|object $definition classname or object
	 *
	 * @return array
	 */
	/*
	public static function getDefinitionProps($definition)
	{
		...
		if (\is_string($definition)) {
			...
			foreach ($classes as $class) {
				if (\class_exists($class)) {
					\bdk\Debug::_warn('class exists!', $class);
					$definition = new $class();
					// $this->debug->info('have field definition class');
					break;
				}
			}
		}
		if (\is_object($definition)) {
			if (\method_exists($definition, 'getDefaultProps')) {
				$propsDefinition = $definition->getDefaultProps();
			}
			$methods = array('build','getValFormatted','getValRaw','setValRaw','validate');
			foreach ($methods as $method) {
				if (!isset($propsDefinition[$method])) {
					$propsDefinition[$method] = \method_exists($definition, $method)
